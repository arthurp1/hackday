...
import { Users, Trophy, Tag, ExternalLink } from 'lucide-react';
...
const ProjectBrowser: React.FC<ProjectBrowserProps> = ({ onClose }) => {
  ...
  const [filter, setFilter] = useState<'all' | 'bounty' | 'challenge'>('all');
  ...
  return (
    <div className="p-3 bg-black/40 backdrop-blur-xl border border-cyan-500/30 rounded-lg max-h-[80vh] overflow-y-auto">
      {onClose && (
            ...
            <div key={project.id} className="p-3 bg-black/20 rounded-lg border border-white/10">
              ...
              <div className="mb-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-gray-400">Team ({teamMembers.length})</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {teamMembers.map(member => (
                    <div key={member.id} className="flex items-center gap-1.5 px-2 py-1 bg-blue-500/10 rounded text-xs">
              ...
              <div className="flex gap-1.5">
                {project.demoUrl && (
                  <a
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 bg-green-500/20 border border-green-500/30 rounded text-green-400 hover:bg-green-500/30 transition-colors text-xs"
                  >
                ...
                {project.videoDemo && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 px-2.5 py-1 bg-red-500/20 border border-red-500/30 rounded text-red-400 hover:bg-red-500/30 transition-colors text-xs"
                  >
                    <ExternalLink className="w-3 h-3" />
                    Video
                  </a>
                ...
                {project.slides && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 px-2.5 py-1 bg-blue-500/20 border border-blue-500/30 rounded text-blue-400 hover:bg-blue-500/30 transition-colors text-xs"
                  >
                    <ExternalLink className="w-3 h-3" />
                    Slides
                  </a>
